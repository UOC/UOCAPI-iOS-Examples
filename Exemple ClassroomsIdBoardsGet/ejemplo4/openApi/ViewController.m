...
{
    // Si estem autenticats iniciem la vista de classrooms
    if (self.loadedAuth == 1) {
        ClassroomsViewController *classroomsViewController = [self.storyboard instantiateViewControllerWithIdentifier:@"ClassroomsView"];
        classroomsViewController.auth = self.auth;
        [self.navigationController pushViewController:classroomsViewController animated:YES];
        self.loadedAuth = 0;
    } else {
        [self signOut];
    }
